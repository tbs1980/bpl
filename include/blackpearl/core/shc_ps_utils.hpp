...
pow_spec<real_scalar_type> extract_pow_spec(
    ...
    std::size_t const m_max = shc.m_max();
    std::size_t const num_fields = shc.num_fields();
...
sph_hrm_coeffs<real_scalar_type> create_gauss_sph_hrm_coeffs(
    ...
    std::size_t const l_max = ps.l_max();
    std::size_t const m_max = l_max;
    std::size_t const num_fields = ps.num_fields();
    ...
    for(std::size_t mtpl_l = 0; mtpl_l <= l_max; ++mtpl_l){
        ...
        std::size_t info = cholesky_decompose( ps_mat_l, ps_chol_l );
        BOOST_ASSERT_MSG(
            info == 0,
        ...
        if (info == 0) {
            ...
            for(std::size_t mtpl_m = 1; mtpl_m <= mtpl_l; ++mtpl_m) {
                for(std::size_t fld_i = 0; fld_i < num_fields; ++fld_i){
                    real_scalar_type re = norm_dist(rng);
...
void convert_to_real_vector(
    ...
    std::size_t num_real_alms = shc.num_real_indep_coeffs(
        shc.num_fields(),
        shc.l_max(),
        shc.m_max()
    );

    std::size_t num_real_cls = pspec.num_real_indep_coeffs(
        pspec.num_fields(),
        pspec.l_max()
    );
